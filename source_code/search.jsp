...
<%
    ...
    if(category.equals("0")){
        strSQL = "SELECT * FROM doc_table WHERE doc_title LIKE ?" ;
        pstmt0 = conn.prepareStatement(strSQL);

        pstmt0.setString(1, "%"+search_text+"%");
    }
    else{
        strSQL = "SELECT  * FROM doc_table WHERE doc_type = ? AND doc_title LIKE ?";
        pstmt0 = conn.prepareStatement(strSQL);

        pstmt0.setInt(1, Integer.parseInt(category));
        pstmt0.setString(2, "%"+search_text+"%");
    }
...
<article id="main_con">
    <h1>검색결과</h1>
    <form method="POST" action="search.jsp?category=<%=category%>" class="search_bar">
			<span class='search_window'>
                <input type="hidden" name="category" value="<%=category%>">
				<input type='text' name="search_text" class='input_text' value="<%=search_text%>" >
			</span>
    ...
    <ol class="table_contents">
        ...
        <%
            if (lastRow > 0) {
                if(category.equals("0")) {
                    strSQL = "SELECT * FROM doc_table WHERE doc_title LIKE ? ORDER BY doc_UID DESC";
                    pstmt0 = conn.prepareStatement(strSQL);
                    ...
                    strSQL = "SELECT * FROM doc_table WHERE doc_type = ? AND doc_title LIKE ? ORDER BY doc_UID DESC";
                    pstmt0 = conn.prepareStatement(strSQL);
    ...
    <div class="page_indicate">
        <%
            ...
            if (lastRow > 0) {
                ...
                if (currentPage > 1) {
        %>
        <a href="listboard.jsp?pageNum=<%=currentPage-1%>&category=<%=category%>">[이전]</a>
        <%
            }
            while (setPage <= lastPage) {
        %>
        <a href="listboard.jsp?pageNum=<%=setPage%>&category=<%=category%>">[<%=setPage%>]</a>
        <%
                setPage = setPage + 1;
            }
            if (lastPage > currentPage) {
        %>
        <a href="listboard.jsp?pageNum=<%=currentPage+1%>&category=<%=category%>">[다음]</a>
        <%
                }
            }
        %>
